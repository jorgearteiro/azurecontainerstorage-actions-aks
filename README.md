...
CONTROLPLANE_IDENTITY_NAME="aks-storage-controlplane-mi"
KUBELET_IDENTITY_NAME="aks-storage-kubelet-mi"
```
...
Instead of authenticating to Azure Storage with a storage account key, this solution uses a **user-assigned managed identity** that is attached to the AKS node pool (the kubelet identity). The Azure Files CSI driver uses this identity to mount the SMB share with no keys or Kubernetes secrets involved.
...
This solution uses **identity-based (keyless) authentication**, so the storage account is created with shared key access **disabled** and SMB OAuth **enabled**. The file share is created and mounted using Azure RBAC and the managed identity — no storage account keys are ever used.
...
No storage secret is required — the Azure Files share is mounted using the kubelet managed identity (see the storage account steps above). Create the runners namespace that the manifests and GitHub App secret will use:
...
volumeAttributes:
  ...
  storageAccount: fileshareactions    # storage account name; must match the account created above
